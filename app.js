// app.js
// V7: Fatigue Control, Confidence Scoring, Weak Area Analytics
...
// Calculate how much effort you've expended *today* based on task difficulty
...
function renderApp() {
    ...
    const todayEffort = getTodayEffort();
    
    // Kept for the visual warning, but it no longer locks the UI
    const isBurnedOut = todayEffort >= MAX_DAILY_EFFORT;
    ...
    learningPath.forEach((dayData) => {
        ...
        // Hardcoded to always be active and unlocked
        let status = 'active'; 
        ...
        // Click listener is always active now
...
function updateAnalytics(effort, totalConf, confCount, weakAreas) {
    ...
    if (weakAreas.length === 0) {
        weakContainer.innerHTML = `<span style="font-size: 0.9rem; color: var(--text-muted);">No weak areas detected yet. Mark scores 1 or 2 to track.</span>`;
    } else {
